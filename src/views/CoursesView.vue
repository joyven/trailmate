<template>
  <div class="min-h-screen bg-gray-50">
    <!-- Hero Section -->
    <section
      class="relative h-96 flex items-center justify-center overflow-hidden"
    >
      <div class="absolute inset-0 z-0">
        <img
          src="https://images.unsplash.com/photo-1545569341-9eb8b30979d9?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80"
          alt="おすすめコース"
          class="w-full h-full object-cover"
        />
        <div class="absolute inset-0 bg-black bg-opacity-50"></div>
      </div>
      <div class="relative z-10 text-center text-white px-4">
        <h1 class="text-5xl md:text-6xl font-bold mb-4">おすすめコース</h1>
        <p class="text-xl md:text-2xl">
          倉敷を楽しむための最適な観光ルートをご紹介します。
        </p>
      </div>
    </section>

    <!-- Main Content -->
    <section class="py-16">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Course Options -->
        <div class="mb-16">
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <!-- Course 1 -->
            <div
              class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow"
            >
              <img
                src="/images/courses/2d4a0f6717f5f1cb5573774536f55475c8db553f.png"
                alt="コース1"
                class="w-full h-64 object-cover"
              />
              <div class="p-6">
                <h3 class="text-2xl font-bold text-gray-900 mb-4">コース1</h3>
                <div class="space-y-4 mb-6">
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">1</span>
                    </div>
                    <span class="font-medium">倉敷駅</span>
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">2</span>
                    </div>
                    <span class="font-medium">美観地区</span>
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">3</span>
                    </div>
                    <span class="font-medium">大原美術館</span>
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">4</span>
                    </div>
                    <span class="font-medium">倉敷川を散歩</span>
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">5</span>
                    </div>
                    <span class="font-medium">倉敷駅に戻る</span>
                  </div>
                </div>

                <div class="bg-gray-50 p-4 rounded-lg mb-6">
                  <h4 class="font-bold text-lg mb-2 text-gray-900">
                    定番の遊びコース
                  </h4>
                  <p class="text-gray-700 text-sm">
                    （初めて訪れ、時間が限られている観光客向け）<br />
                    「最も有名で代表的な観光地をカバーし、路線を最適化して交通時間を短縮する」
                  </p>
                </div>

                <div class="flex items-center justify-between">
                  <span class="text-gray-600 text-sm">半日コース</span>
                  <button
                    class="bg-kurarishu-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-kurarishu-blue-700 transition-colors"
                  >
                    詳細を見る
                  </button>
                </div>
              </div>
            </div>

            <!-- Course 2 -->
            <div
              class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow"
            >
              <img
                src="/images/courses/2d4a0f6717f5f1cb5573774536f55475c8db553f.png"
                alt="コース2"
                class="w-full h-64 object-cover"
              />
              <div class="p-6">
                <h3 class="text-2xl font-bold text-gray-900 mb-4">コース2</h3>
                <div class="space-y-4 mb-6">
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">1</span>
                    </div>
                    <span class="font-medium">大原美術館</span>
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">2</span>
                    </div>
                    <span class="font-medium">倉敷民藝館</span>
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">3</span>
                    </div>
                    <span class="font-medium"
                      >陶磁器店/編み物店/カウボーイ店</span
                    >
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">4</span>
                    </div>
                    <span class="font-medium">手作り体験を享受</span>
                  </div>
                </div>

                <div class="bg-gray-50 p-4 rounded-lg mb-6">
                  <h4 class="font-bold text-lg mb-2 text-gray-900">
                    深い文化探索
                  </h4>
                  <p class="text-gray-700 text-sm">
                    （文化愛好家、歴史ファン、スロートラベラー向け）<br />
                    「一部の人の流れを避け、路地、博物館、作業場に入り、体験と学習を強調する。」
                  </p>
                </div>

                <div class="flex items-center justify-between">
                  <span class="text-gray-600 text-sm">1日コース</span>
                  <button
                    class="bg-kurarishu-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-kurarishu-blue-700 transition-colors"
                  >
                    詳細を見る
                  </button>
                </div>
              </div>
            </div>

            <!-- Course 3 -->
            <div
              class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow"
            >
              <img
                src="/images/courses/2d4a0f6717f5f1cb5573774536f55475c8db553f.png"
                alt="コース3"
                class="w-full h-64 object-cover"
              />
              <div class="p-6">
                <h3 class="text-2xl font-bold text-gray-900 mb-4">コース3</h3>
                <div class="space-y-4 mb-6">
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">1</span>
                    </div>
                    <span class="font-medium">着物の体験を享受</span>
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">2</span>
                    </div>
                    <span class="font-medium"
                      >友達と古い建物の前で写真を撮る</span
                    >
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">3</span>
                    </div>
                    <span class="font-medium">古い通りで写生する</span>
                  </div>
                  <div class="flex items-center space-x-2 text-blue-600">
                    <svg
                      class="w-6 h-6"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M13 7l5 5m0 0l-5 5m5-5H6"
                      ></path>
                    </svg>
                  </div>
                  <div class="flex items-center space-x-2 text-green-600">
                    <div
                      class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center"
                    >
                      <span class="text-green-600 font-bold text-sm">4</span>
                    </div>
                    <span class="font-medium"
                      >川辺の喫茶店でおしゃべりをする</span
                    >
                  </div>
                </div>

                <div class="bg-gray-50 p-4 rounded-lg mb-6">
                  <h4 class="font-bold text-lg mb-2 text-gray-900">
                    らくにコースを歩く
                  </h4>
                  <p class="text-gray-700 text-sm">
                    （写真愛好家、ソーシャルメディアの達人、親友が旅行に向け）<br />
                    「友達と一緒に美しい思い出を残すことは、旅行の中で最も重要な部分です」
                  </p>
                </div>

                <div class="flex items-center justify-between">
                  <span class="text-gray-600 text-sm">半日コース</span>
                  <button
                    class="bg-kurarishu-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-kurarishu-blue-700 transition-colors"
                  >
                    詳細を見る
                  </button>
                </div>
              </div>
            </div>
          </div>
        </div>

        <!-- Course Benefits -->
        <div class="mb-16">
          <h2 class="text-3xl font-bold text-gray-900 mb-8">
            コース選びのポイント
          </h2>

          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-white rounded-xl shadow-lg p-6 text-center">
              <div
                class="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4"
              >
                <svg
                  class="w-8 h-8 text-blue-600"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
                  ></path>
                </svg>
              </div>
              <h3 class="font-bold text-lg mb-2">時間効率化</h3>
              <p class="text-gray-600 text-sm">最短時間で倉敷の魅力を楽しむ</p>
            </div>

            <div class="bg-white rounded-xl shadow-lg p-6 text-center">
              <div
                class="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"
              >
                <svg
                  class="w-8 h-8 text-green-600"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"
                  ></path>
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
                  ></path>
                </svg>
              </div>
              <h3 class="font-bold text-lg mb-2">spots網羅</h3>
              <p class="text-gray-600 text-sm">主要スポットを効率的に回る</p>
            </div>

            <div class="bg-white rounded-xl shadow-lg p-6 text-center">
              <div
                class="w-16 h-16 bg-yellow-100 rounded-full flex items-center justify-center mx-auto mb-4"
              >
                <svg
                  class="w-8 h-8 text-yellow-600"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"
                  ></path>
                </svg>
              </div>
              <h3 class="font-bold text-lg mb-2">満足度向上</h3>
              <p class="text-gray-600 text-sm">あなたの好みに 맞는体験</p>
            </div>

            <div class="bg-white rounded-xl shadow-lg p-6 text-center">
              <div
                class="w-16 h-16 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-4"
              >
                <svg
                  class="w-8 h-8 text-purple-600"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"
                  ></path>
                </svg>
              </div>
              <h3 class="font-bold text-lg mb-2">コスト最適化</h3>
              <p class="text-gray-600 text-sm">経済的で効率的な旅</p>
            </div>
          </div>
        </div>

        <!-- Map Integration -->
        <div class="mb-16">
          <h2 class="text-3xl font-bold text-gray-900 mb-8">コースマップ</h2>

          <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="aspect-w-16 aspect-h-9">
              <div
                class="w-full h-96 bg-gray-200 flex items-center justify-center"
              >
                <div class="text-center">
                  <svg
                    class="w-24 h-24 text-gray-400 mx-auto mb-4"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      stroke-width="2"
                      d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"
                    ></path>
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      stroke-width="2"
                      d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
                    ></path>
                  </svg>
                  <p class="text-gray-600 text-lg font-medium">
                    インタラクティブマップ
                  </p>
                  <p class="text-gray-500">コースルートとspotsを表示します</p>
                </div>
              </div>
            </div>
          </div>
        </div>

        <!-- Call to Action -->
        <div class="bg-kurarishu-blue-50 rounded-2xl p-8 text-center">
          <h3 class="text-2xl font-bold text-gray-900 mb-4">
            最適なコースが見つかりましたか？
          </h3>
          <p class="text-gray-700 mb-6 max-w-2xl mx-auto">
            あなたの兴趣とニーズに合わせて、最小限の時間仓
            максимаの仓敷の魅力をお楽しみください。
          </p>
          <div class="space-x-4">
            <button
              class="bg-kurarishu-blue-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-kurarishu-blue-700 transition-colors"
            >
              地図でコースを表示
            </button>
            <router-link
              to="/spots"
              class="inline-block border-2 border-kurarishu-blue-600 text-kurarishu-blue-600 px-8 py-3 rounded-lg font-semibold hover:bg-kurarishu-blue-600 hover:text-white transition-colors"
            >
              スポットを詳しく見る
            </router-link>
          </div>
        </div>
      </div>
    </section>
  </div>
</template>

<script setup lang="ts">
// Component logic here
</script>
